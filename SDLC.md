...
| Review loop | The reviewer reads the diff against the `ot-code-review` checklist and approves or requests changes. Requested changes are addressed (`ot-auto-continue-pr` resumes agent PRs from the tracking plan) and the PR is re-reviewed until approved. A user-facing change also gets a design pass: `ot-ux-review-pr` walks the changed screens and reports findings ranked by user impact. That pass is advisory — it informs the review, it does not hold the merge. | `ot-auto-review-pr` (single PR), `ot-review-prs` (sweep), `ot-ux-review-pr` (design pass), or a human | Approving review submitted |
