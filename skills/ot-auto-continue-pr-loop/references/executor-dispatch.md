...
```markdown
You are an executor for ot-auto-continue-pr-loop PR #{prNumber}. Implement exactly one Step.
...
The creator counterpart (`ot-auto-create-pr-loop`) inherits this pattern when driving multiple Steps in a single invocation.
