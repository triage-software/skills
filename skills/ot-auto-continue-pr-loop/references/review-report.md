...
Detailed procedure for the single automated review loop of `ot-auto-continue-pr-loop`.
...
3. Loop until `ot-auto-review-pr` returns a clean verdict or the remaining findings are non-actionable (out-of-scope, false positive) and explicitly documented in the summary comment you post in step 9.
...
## ot-auto-continue-pr-loop specifics

- The claim check inside `ot-auto-review-pr` will recognize that the current user already owns the `in-progress` lock (claimed in step 1), so it proceeds as re-entry without re-claiming.
