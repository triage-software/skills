...
Prefer the explicit `Tracking plan:` line in the PR body (written by `ot-auto-create-pr-loop`): from the `body` already fetched by the step 1 **get-pr** (no second fetch), take the first line matching `^Tracking (plan|run folder):` (e.g. pipe the body through `grep -E '^Tracking (plan|run folder):' | head -n1`).
