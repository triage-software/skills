...
```markdown
You are an executor for ot-auto-create-pr-loop run {SLUG}. Implement exactly one Step.
...
Sibling auto-skills (`ot-auto-continue-pr-loop`, `ot-auto-update-changelog`) inherit this pattern when driving multiple Steps in a single invocation.
