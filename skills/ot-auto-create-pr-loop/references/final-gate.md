...
- Run the repo's integration suite via the `ot-integration-tests` skill (running-only mode), full scope. Capture the report summary and save it as `final-gate-artifacts/integration-report-summary.log`. On failure, fix forward with new Steps; never skip a failing suite.
