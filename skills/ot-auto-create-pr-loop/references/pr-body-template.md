...
For a spec-implementation run, use these exact tracking lines:
...
The Progress pointer refers to PLAN.md's Tasks table (`todo` / `done`). Set
`Status: complete` once every row is `done` and the final gate passes. Keep the
HANDOFF.md and NOTIFY.md links in the run folder; link them in the PR only when
a resume needs them. Simple runs omit tracking fields per their mode contract.
