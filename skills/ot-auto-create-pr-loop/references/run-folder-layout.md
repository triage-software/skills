...
This section is the layout contract; `ot-auto-continue-pr-loop` parses these files to resume a run.
...
Do not pre-create `checkpoint-*-checks.md` or `checkpoint-*-artifacts/` — each checkpoint writes its own files when it fires. This guarantees that if anything later crashes, `ot-auto-continue-pr-loop` can find `PLAN.md`, `HANDOFF.md`, and `NOTIFY.md` via the remote branch.
