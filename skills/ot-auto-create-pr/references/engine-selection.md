...
How `ot-auto-create-pr` decides whether to run plain or hand the run to `ot-auto-create-pr-loop`. Orchestrating skills (`ot-auto-implement-spec`, `ot-auto-fix-issue`'s feature route) delegate to this rule by invoking `ot-auto-create-pr`, forwarding `--loop` verbatim only when the user passed it — they never add the flag on their own.
...
Hand off to `ot-auto-create-pr-loop` only when at least one holds:
...
Nothing else selects the loop: not UI work (plain runs post screenshots via `ot-auto-qa-pr` / **attach-image-evidence**), not "might not finish in one pass" (`ot-auto-continue-pr` resumes a plain run fine), not subjective "feels large".
...
The engine is decided **before** any plan is committed, because the two engines consume different plan formats: plain → the standard Progress-tracked execution plan (per `ot-auto-create-pr` step 4); loop → the run-folder in `ot-auto-create-pr-loop`'s format (`PLAN.md` Tasks table + `HANDOFF.md`/`NOTIFY.md`). A run's format is therefore fixed at creation — resume plain runs with `ot-auto-continue-pr`, run-folder runs (a `Tracking run folder:` line in the PR body, or a run-folder tracking path) with `ot-auto-continue-pr-loop`. Never re-apply the threshold to a run that already has artifacts.
