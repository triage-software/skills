...
The path `ot-auto-fix-issue` takes when its required argument is a free-form
...
1. **Invoke the `ot-prepare-issue` skill verbatim** with the description as its
   ...
   duplicate. When `ot-prepare-issue` is not installed, stop and name it as the
   missing chain skill.

2. **Run it under this chain's autonomous contract.** `ot-prepare-issue` is an
...
4. **Respect an authored spec PR.** When `ot-prepare-issue` took its
...
`ot-auto-write-spec` under the same automation identity is re-entry), and
step 2 classifies it — `ot-prepare-issue`'s category label feeds the
