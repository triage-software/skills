---
name: ot-auto-fix-pr
description: Drive an open PR to merge-ready from its number — merges the latest base, then loops review-autofix (ot-auto-review-pr), built-in CI stabilization, and UI verification (ot-auto-qa-pr) until approvable, green, and QA-evidenced. A --ci-only mode drives just CI green on a PR or a plain branch. Files follow-up issues for nits, normalizes labels, hands off to ot-approve-merge-pr — never merges itself. Use for "get PR 123 merge-ready".
---
...
stays with `ot-approve-merge-pr` / `ot-merge-buddy` behind the QA gate.

This skill is an **orchestrator**: it holds the outer claim and coordinates
`ot-auto-review-pr` (review + autofix + conflict/fork handling), `ot-auto-qa-pr`
(UI QA), and `ot-followup-issue-from-pr` (nit follow-ups), plus the **built-in CI
...
`ot-auto-fix-issue` (issue-side chain).

A **`--ci-only` mode** drives just CI green — on a PR (`ot-auto-fix-pr 123
--ci-only`) or on a plain branch with no PR yet (`ot-auto-fix-pr --ci-only --branch
...
- Orchestrate, don't reinvent: delegate review/autofix/conflict/fork handling to `ot-auto-review-pr`, UI QA to `ot-auto-qa-pr`, and nit follow-ups to `ot-followup-issue-from-pr`; invoke each verbatim and pass its outputs on. CI stabilization is built in (`references/stabilize-ci.md`) — follow that procedure rather than re-deriving it.
...
- **Conflicts first, then findings, then CI** — the loop's stage order is mandatory, and both earlier stages are delegated to `ot-auto-review-pr --autofix` rather than re-implemented here. CI is never stabilized on a branch that is still conflicted or still carries actionable review findings.
...
- **Fork supersede/credit**: when the review step carries a fork PR forward into a replacement PR, preserve the `Supersedes #{prNumber}` line, credit the original author, and reassign the replacement to them — per `ot-auto-review-pr`'s fork flow and the Supersede Credit Rule checks in `references/pr-finalize.md`.
...
- **Never merges, never fakes QA**: this skill leaves the PR merge-ready and hands off; it never squash-merges and never adds `qa-approved` (the QA gate and `ot-approve-merge-pr` own that). When the QA gate is on, a `needs-qa` PR stays unmergeable until a QA reviewer signs off.
