...
Two related things `ot-auto-fix-pr` runs: the **outer loop** that drives a PR to
...
1. **Conflicts, then review findings — `ot-auto-review-pr {prNumber} --autofix`**
...
3. **Verify UI — `ot-auto-qa-pr {prNumber}`** — only when the diff touches a
...
7. **Wait for CI — bounded**: **watch-run** on the new runs (or poll
   **get-pr-checks**) until they settle, for at most `CI_MAX_WAIT_MINUTES`
   ...
   without CI — the hand-off to `ot-approve-merge-pr` still requires genuinely green
   required checks.
...
tracked follow-up (via `ot-followup-issue-from-pr` or **create-issue**): test name,
...
- `ot-auto-review-pr` returns approvable — no conflicts and no actionable blockers.
