...
The one procedure for turning a `{spec}` reference into a concrete spec file. Shared contract: `ot-auto-create-pr --spec` and `ot-auto-fix-issue`'s feature-route spec lookup follow this same order — update all consumers together.
