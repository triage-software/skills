...
This skill never boots the app itself — `ot-prepare-test-env` provisions a runnable
...
In PR mode this skill consumes a `{prNumber}` (the `PR:` reference line a PR-producing skill emitted) and posts screenshot QA evidence back to that existing PR; it is read-only on source and never opens a PR. PR mode ends by reporting the `PR:` / `Issue:` chaining reference lines; in local mode the artifacts folder is the deliverable. Companion skills: `ot-auto-review-pr` (review-first gate), `ot-prepare-test-env` (boots/provisions the instance and browser), `ot-integration-tests` (follow-up automated UI test), `ot-setup-agent-pipeline` (installs a missing browser provider) — each runs verbatim; a missing required one stops the run naming the skill to install.
...
3. **Review-first gate (PR mode only).** QA runs **after** code review. Check the
   ...
   - **Not reviewed** — no approve/changes-requested `reviewDecision` (null /
     ...
     invoke **`ot-auto-review-pr {prNumber}`** verbatim first (it re-enters the
...
5. **Detect whether the change already ships a UI test.** Look in the diff for
   ...
   (discover it the way `ot-integration-tests` does: an `__integration__/`,
...
7. **Boot the app via `ot-prepare-test-env`.** Never boot by hand. Invoke the
   `ot-prepare-test-env` skill (mode `auto`; `--no-ephemeral` when the app needs
...
12. **Follow-up UI-test scenario (only when `HAS_UI_TEST` from step 5 is
    ...
    `ot-integration-tests` — a second PR comment in PR mode (**comment-pr**), or
...
13. **Labels, teardown, and lock release.**
    ...
    **Teardown (run in a `trap`/finally):**
    - Tear down the environment only if this run started it and `--keep-env` was
      not set — invoke `ot-prepare-test-env --stop`. Otherwise leave it running
      for reuse.
    ...
    - **PR mode, inherited chain lock** (re-entry — a flow runner such as
      `ot-auto-fix-issue` or `ot-auto-fix-pr` handed the lock off to this run):
...
- Boot the app only through `ot-prepare-test-env`; reuse a running environment;
