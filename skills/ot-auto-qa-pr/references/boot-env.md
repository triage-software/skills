# Boot the app via ot-prepare-test-env

Detailed procedure for step 7 of `ot-auto-qa-pr`. Do not boot the app by hand. Invoke the `ot-prepare-test-env` skill (mode `auto`; pass `--no-ephemeral` when the app clearly needs no backing services). It discovers or provisions a runnable instance, installs the configured browser provider when missing, and writes the environment descriptor. Then read the descriptor:
...
setup error — invoke `ot-setup-agent-pipeline` to install it, then retry.
...
same way, without quoting them back. If `ot-prepare-test-env` reports the app
