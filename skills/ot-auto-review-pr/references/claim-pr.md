...
| In progress, current user owns the lock | — | Re-entry (incl. a chain hand-off lock from `ot-open-pr --handoff` or a flow runner's outer claim): post the take-over comment naming this skill **before any review work**, then proceed without re-claiming |
...
```text
🤖 `ot-auto-review-pr` started by @{CURRENT_USER} at {timestamp}. Other auto-skills will skip this PR until the lock is released.
```
...
```text
🤖 `ot-auto-review-pr` completed: {VERDICT}. Lock released.
```
...
```text
🤖 `ot-auto-review-pr` completed: {VERDICT}. Lock retained — chain continues.
```

The chain's driving skill (e.g. `ot-auto-fix-issue`) releases the lock at the end of its run.
