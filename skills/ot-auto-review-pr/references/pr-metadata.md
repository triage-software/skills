...
Detailed procedure for steps 2–3 of `ot-auto-review-pr`.
...
fixing chains (`ot-auto-fix-pr`, `ot-auto-fix-issue`) exist to clear it. Collect it from all three
...
**Severity.** Map the reviewer's own wording onto the `ot-code-review` scale — "must" / "required" /
