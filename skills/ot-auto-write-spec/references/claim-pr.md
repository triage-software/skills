...
- Release/handback happens in step 8. Delegate to `ot-open-pr` when it ran — its hand-off step already performs the handback and lock release. Inline otherwise:
  ...
  3. Remove the `in-progress` label via the guard, then post the release comment: `` 🤖 `ot-auto-write-spec` — completed: opened {PR_URL}. Lock released. `` (substitute `(no PR — aborted)` when no PR opened).
