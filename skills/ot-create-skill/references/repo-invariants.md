...
The hard, repo-specific constraints `ot-create-skill` bakes into every skill it
authors or splits. These are what make a skill "OT-shaped" rather than generic.
...
- **Frontmatter**: every `skills/<name>/SKILL.md` starts with a `---` frontmatter
  ...
  `description`.
- **Product-agnostic content** (behavior, not the `ot-` name prefix, which is
...
- **Config load**: the standard `.ai/agentic.config.json` snippet from the
  `ot-setup-agent-pipeline` skill, resolving `BASE_BRANCH`, `LABELS_ENABLED`,
