...
> the `ot-setup-agent-pipeline` skill. If either is missing, run the
> `ot-setup-agent-pipeline` skill now (interactively with a user present,
...
## Communication contract (required for every `ot-auto-*` skill)
...
- Tracker comments with stable idempotent markers — `` 🤖 `<skill-name>` — <purpose> ``
  ...
  (autonomous defaults), run summary (`ot-auto-create-pr` step-12 structure),
...
- Labels only through the descriptor guards, per the canonical rules
  (`ot-open-pr` step 6 / this skill's copy in `references/pr-finalize.md`);
...
- Every `references/...` pointer must resolve; cross-skill pointers are written
  as explicit `ot-<skill>/references/<file>.md` paths.
