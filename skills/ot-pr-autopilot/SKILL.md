...
to the existing `ot-*` skills and never re-implements their logic.

It is the dispatcher that sits above `ot-auto-continue-pr` (finish the
implementation), `ot-auto-fix-pr` (drive to merge-ready), `ot-auto-qa-pr` (UI
evidence), and `ot-approve-merge-pr` (merge). Use those directly when you
...
- `--allow-merge` (optional) — permit the chain to end in an actual merge via
  `ot-approve-merge-pr`. **Off by default**: the run stops at merge-ready.
- `--force` (optional) — take over an `in-progress` claim held by another actor.
- `--max-iterations <n>` (optional) — forwarded to `ot-auto-fix-pr`. Default `3`.
...
`ot-auto-review-pr` flow rather than here. It ends by reporting the `PR:` and
`Issue:` chaining reference lines. Companion skills, each invoked **verbatim**:
`ot-auto-continue-pr`, `ot-auto-continue-pr-loop`, `ot-auto-fix-pr`,
`ot-auto-review-pr`, `ot-auto-qa-pr`, `ot-followup-issue-from-pr`,
`ot-approve-merge-pr`. A missing companion stops the run and names the skill to
install — never improvise a replacement for it. Only a skill this run can
actually dispatch belongs on that list: `ot-merge-buddy` scans the whole open
...
0. **Agentic setup** — follow `references/agentic-setup.md`: load
   ...
   `ot-setup-agent-pipeline` when missing), apply the repo-local override
...
- **Never merge implicitly.** The chain stops at merge-ready unless
  ...
  `ot-approve-merge-pr` owns the merge.
...
- **Spec-only design PRs stay design-only.** Implementation ships on its own PR
  via `ot-auto-implement-spec`; never grow a design PR into implementation here.
