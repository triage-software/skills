...
The step 3 branch of `ot-prepare-issue`: when the duplicate/spec search (steps 1–2)
...
which `ot-prepare-issue` creates a PR, and the PR contains a **spec document only**
...
2. **Delegate spec authoring to `ot-auto-write-spec {issueId}`** — the dedicated
   ...
   writes the spec via `ot-spec-writing --autonomous` **including its Open
...
commit. Implementation happens later via `ot-auto-implement-spec {SPEC_PATH}` or
`ot-auto-fix-issue {issueId}` — both keep the spec PR design-only and ship the
implementation on its own PR referencing it (`Refs #{specPr}`). Report both the
issue and the spec PR in step 6.

## Guardrails

- The PR is design-only. Delegate to `ot-auto-write-spec` (which never
  implements) — never to `ot-auto-fix-issue` / `ot-auto-implement-spec` on
  this path; those implement.
- Everything else about `ot-prepare-issue` stays tracker-first: duplicate search,
