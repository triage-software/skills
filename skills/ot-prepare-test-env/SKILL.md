...
1. **Execute the saved entrypoint (every run — "Phase 1" in the references).**
   ...
   if [ -f "$UP_SCRIPT" ] && grep -q 'ot-prepare-test-env: generated entrypoint' "$UP_SCRIPT" \
      && [ "$REGENERATE" != 1 ]; then
   ...
   if ((Test-Path $UpScript) -and
       (Select-String -Quiet 'ot-prepare-test-env: generated entrypoint' $UpScript) -and
       -not $Regenerate) {
   ...
   - **Script succeeds** → read `baseUrl` from `$ENV_DESCRIPTOR`, print the
     ...
     run so consumers (`ot-auto-qa-pr`, `ot-integration-tests`) attach to the
   ...
   The marker line (`# ot-prepare-test-env: generated entrypoint`) is how the
...
3. **Bake every lesson back into the scripts (self-improvement).** **Any
   ...
   3. Append the exact working command chain (and the failure it prevents) to
      the repo-local skill at `.ai/skills/ot-prepare-test-env/SKILL.md` —
