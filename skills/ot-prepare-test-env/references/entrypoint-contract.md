...
`ot-prepare-test-env` body during Phase 2 step 2.3 (write the scripts); the fast
...
1. **Marker + parameters.** First lines after the shebang:
   ...
   ```sh
   #!/bin/sh
   # ot-prepare-test-env: generated entrypoint (contract v2)
   # regenerate with: ot-prepare-test-env --regenerate
   # history:
