...
- `engine.loopStepThreshold` — the Step count above which `ot-auto-create-pr` hands a run off to `ot-auto-create-pr-loop` (default 20). Raise it to keep more runs on the cheaper plain engine; `--loop` always forces the loop regardless.
...
- `paths.specs` — where feature specifications live (default `.ai/specs`). Spec filenames follow `{YYYY-MM-DD}-{kebab-case-title}.md`. `ot-spec-writing` writes here, `ot-prepare-issue` links from here, `ot-followup-issue-from-pr` checks here first in design-doc mode, and `ot-brainstorm` writes handoff briefs under `<paths.specs>/briefs/`.
- `paths.scripts` — where reusable environment scripts are generated (default `.ai/scripts`); `ot-prepare-test-env` writes the env bring-up/teardown scripts here.
