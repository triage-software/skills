...
setup authority is `ot-setup-agent-pipeline`.

## Preflight

1. Load `.ai/agentic.config.json` via the standard snippet. Config or
   `$TRACKER_FILE` missing → run `ot-setup-agent-pipeline` now (interactively
...
3. Apply a repo-local `.ai/skills/ot-ux-review-pr/SKILL.md` as an extension
...
- As part of preflight step 4, load the design contract when present:
  ...
  ```bash
  # Written by ot-ux-setup; absent is not an error.
